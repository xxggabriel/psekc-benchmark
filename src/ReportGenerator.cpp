...
void ReportGenerator::print_table(const std::vector<BenchmarkResult>& results) {
    ...
    std::cout << "-------------------------------------------------------------------------------" << std::endl;
    std::cout << std::setw(20) << "Plataforma" << "| "
              << std::setw(15) << "Tamanho Seq." << "| "
              << std::setw(8)  << "Threads"      << "| "
              << std::setw(10) << "Tempo (s)"    << "| "
              << std::setw(17) << "Speedup vs Seq." << "| "
              << "Eficiência (%)" << std::endl;
    std::cout << "---------------------|---------------|--------|-----------|-----------------|---------------" << std::endl;

    for (const auto& res : results) {
        std::cout << std::setw(20) << res.platform << "| "
                  << std::setw(15) << res.sequence_size << "| "
                  << std::setw(8)  << res.num_threads    << "| "
                  << std::setw(10) << res.duration_sec   << "| "
                  << std::setw(17) << (std::to_string(res.speedup).substr(0,5) + "x") << "| ";
...
void ReportGenerator::save_to_csv(const std::vector<BenchmarkResult>& results, const std::string& filename) {
    ...
    std::cout << "\nRelatório de benchmark salvo em '" << filename << "'." << std::endl;
}
